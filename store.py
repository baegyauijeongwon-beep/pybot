    ...
    try:
        ...
        if match_trade:
            ...
            if inv_dict.get(item_name, 0) < trade_count:
                reply(status, f"해당 상품을 {trade_count}개만큼 가지고 있지 않습니다.")
                return
            
            # 타겟 유저 찾기 (아이디로 찾음)
            target_idx = next((i + 2 for i, row in enumerate(user_rows[1:]) if row[0].strip().lower() == target_handle.lower()), -1)
            if target_idx == -1:
                reply(status, f"{target_handle}님은 명단에 없습니다. 다시 확인해주세요.")
                return
            ...
            # 🌟 툿 출력도 '이름'으로 변경!
            reply(status, f"{target_handle}\n\n{target_name}에게 [{item_name} {trade_count}개] 양도 완료")
            return
        # 2-2. 갈레온(재화) 양도 기능
        ...
        if match_money_trade:
            ...
            if transfer_amount <= 0:
                reply(status, "양도할 금액은 1 갈레온 이상이어야 합니다.")
                return
            ...
            if current_money < transfer_amount:
                reply(status, f"갈레온이 부족합니다. (현재 보유: [{current_money:,}] 갈레온)")
                return
            ...
            # ⑥ 완료 영수증 툿 발송 (이름으로 출력!)
            reply(
                status, f"{target_name}에게 [{transfer_amount:,}] 갈레온을 보냈습니다."
            )
            return
